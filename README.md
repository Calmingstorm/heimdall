...
Autonomous executor Discord bot with infrastructure management, 77 tools, and the burden of seeing everything.
...
Every Discord message
  → Codex (with 77 tools + personality in system prompt)
...
Tool packs (opt-in infrastructure tools):
  ...
  Empty config = all 77 tools loaded (backward compatible)
```
...
- **Tools** — browse all 77 tools, toggle tool packs on/off
