...
Autonomous executor Discord bot with infrastructure management, 80 tools, and the burden of seeing everything.
...
Every Discord message
  → Codex (with 80 tools + personality in system prompt)
...
Tool packs (opt-in infrastructure tools):
  ...
  Empty config = all 80 tools loaded (backward compatible)
```
...
- **Tools** — browse all 80 tools, toggle tool packs on/off
