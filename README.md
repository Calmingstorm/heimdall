...
Autonomous executor Discord bot with infrastructure management, 72 tools, and the burden of seeing everything.
...
Every Discord message
  → Codex (with 72 tools + personality in system prompt)
...
Tool packs (opt-in infrastructure tools):
  ...
  Empty config = all 72 tools loaded (backward compatible)
```
...
- **Tools** — browse all 72 tools, toggle tool packs on/off
