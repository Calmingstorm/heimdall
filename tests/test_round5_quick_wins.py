...
5. timeout 280 wrapper for remote claude command
"""
...
# ---------------------------------------------------------------------------
# Fix 5: timeout 280 wrapper for remote claude command
# ---------------------------------------------------------------------------
...
class TestClaudeCodeTimeoutWrapper:
    ...
    async def test_allow_edits_command_has_timeout(self, executor):
        """allow_edits=True path should include 'timeout 280' in the SSH command."""
        call_count = [0]
        tmpdir = "/tmp/claude_code_abcd1234"
        ...
        assert "timeout 280" in cmd_arg

    async def test_readonly_command_has_timeout(self, executor):
        """allow_edits=False (read-only) path should include 'timeout 280' in the SSH command."""
        ...
        assert "timeout 280" in first_call_cmd
