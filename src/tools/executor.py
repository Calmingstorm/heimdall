...
class ToolExecutor:
    ...
    async def _handle_check_docker(self, inp: dict) -> str:
        container = inp.get("container")
        if container:
            safe = shlex.quote(container)
            cmd = f"docker ps -a --filter name={safe} --format 'table {{{{.Names}}}}\\t{{{{.Status}}}}\\t{{{{.Ports}}}}'"
        else:
    ...
    async def _handle_docker_logs(self, inp: dict) -> str:
        container = shlex.quote(inp["container"])
    ...
    async def _handle_docker_compose_action(self, inp: dict) -> str:
        project_dir = shlex.quote(inp["project_dir"])
        action = inp["action"]
        if action not in ("up", "down", "pull", "restart", "build"):
            return f"Invalid action: {action}"
        cmd = f"cd {project_dir} && docker compose"
        if action == "up":
            cmd += " up -d"
        elif action == "down":
            cmd += " down"
    ...
    async def _handle_docker_compose_status(self, inp: dict) -> str:
        project_dir = shlex.quote(inp["project_dir"])
        cmd = f"cd {project_dir} && docker compose ps -a"
        return await self._run_on_host(inp["host"], cmd)

    async def _handle_docker_compose_logs(self, inp: dict) -> str:
        project_dir = shlex.quote(inp["project_dir"])
        lines = min(inp.get("lines", 50), 200)
        cmd = f"cd {project_dir} && docker compose logs --tail {lines}"
        since = inp.get("since")
        if since:
            cmd += f" --since {shlex.quote(since)}"
    ...
    async def _handle_docker_stats(self, inp: dict) -> str:
        container = inp.get("container")
        if container:
            safe = shlex.quote(container)
            cmd = f"docker stats --no-stream --format 'table {{{{.Name}}}}\\t{{{{.CPUPerc}}}}\\t{{{{.MemUsage}}}}\\t{{{{.NetIO}}}}' {safe}"
        else:
    ...
    async def _handle_git_status(self, inp: dict) -> str:
        repo = shlex.quote(inp["repo_path"])
        return await self._run_on_host(inp["host"], f"git -C {repo} status")

    async def _handle_git_log(self, inp: dict) -> str:
        repo = shlex.quote(inp["repo_path"])
        count = min(inp.get("count", 10), 50)
        return await self._run_on_host(
            inp["host"],
            f"git -C {repo} log --oneline --graph -n {count}",
        )

    async def _handle_git_diff(self, inp: dict) -> str:
        repo = shlex.quote(inp["repo_path"])
        commit = inp.get("commit")
        if commit:
            safe_commit = shlex.quote(commit)
            cmd = f"git -C {repo} diff {safe_commit}~ {safe_commit}"
        else:
            cmd = f"git -C {repo} diff"
        return await self._run_on_host(inp["host"], cmd)

    async def _handle_git_show(self, inp: dict) -> str:
        repo = shlex.quote(inp["repo_path"])
        commit = shlex.quote(inp["commit"])
        return await self._run_on_host(inp["host"], f"git -C {repo} show {commit}")

    async def _handle_git_pull(self, inp: dict) -> str:
        repo = shlex.quote(inp["repo_path"])
        return await self._run_on_host(inp["host"], f"git -C {repo} pull")

    async def _handle_git_commit(self, inp: dict) -> str:
        repo = shlex.quote(inp["repo_path"])
        message = shlex.quote(inp["message"])
        files = inp.get("files")
        if files:
            safe_files = " ".join(shlex.quote(f) for f in files)
            cmd = f"git -C {repo} add {safe_files} && git -C {repo} commit -m {message}"
        else:
            cmd = f"git -C {repo} add -A && git -C {repo} commit -m {message}"
        return await self._run_on_host(inp["host"], cmd)

    async def _handle_git_push(self, inp: dict) -> str:
        repo = shlex.quote(inp["repo_path"])
        remote = shlex.quote(inp.get("remote", "origin"))
        branch = inp.get("branch")
        if branch:
            safe_branch = shlex.quote(branch)
            cmd = f"git -C {repo} push {remote} {safe_branch}"
        else:
            cmd = f"git -C {repo} push {remote}"
        return await self._run_on_host(inp["host"], cmd)

    async def _handle_git_branch(self, inp: dict) -> str:
        repo = shlex.quote(inp["repo_path"])
        action = inp["action"]
        branch_name = inp.get("branch_name")
        if action == "list":
            return await self._run_on_host(
                inp["host"], f"git -C {repo} branch -a"
            )
        if not branch_name:
            return "branch_name is required for create and switch actions"
        safe_branch = shlex.quote(branch_name)
        if action == "create":
            cmd = f"git -C {repo} checkout -b {safe_branch}"
        elif action == "switch":
            cmd = f"git -C {repo} checkout {safe_branch}"
        else:
